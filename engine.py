...
def map_transactions(transactions: pd.DataFrame, rules: List[Rule]) -> pd.DataFrame:
    ...
    def _best_rule_by_specificity(hits, desc_lower: str):
        # Score = longest matched keyword length (literal substring); tie -> None
        best = None
        best_score = -1
        tie = False
        for rid in hits:
            ...
            if score > best_score:
                ...
                # if same score but different target, treat as tie
                tie = True
    ...
    for pos, ix in enumerate(tx.index):
        ...
        # If multiple rules match but they map to the same target (category/detail), treat as unambiguous.
        ...
        # Deterministic tie-breaker (kept reviewable):
        # choose the rule with the longest matched keyword, if unique.
        d = str(tx.at[ix, "description"]).lower()
        best = _best_rule_by_specificity(hits, d)
...
def monthly_rollups(mapped_tx: pd.DataFrame, month: str) -> Dict[str, pd.DataFrame]:
    ...
    # Include fully mapped plus deterministic tie-breaker picks (still reviewable)
