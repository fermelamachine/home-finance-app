...
if budgets_df is not None:
    cats_set |= set([str(c).strip() for c in budgets_df["BUDGET CATEGORY"].unique() if str(c).strip() != ""])
if rules_workbook is not None:
    # include categories present in RULES (can include PAYROLL/TRANSFERS)
    try:
        cats_set |= set([str(c).strip() for c in rules_df["BUDGET CATEGORY"].unique() if str(c).strip() != ""])
    except Exception:
        pass
cats_set |= set([str(c).strip() for c in tx_all.get("budget_category", pd.Series(dtype=str)).unique() if str(c).strip() != ""])
...
transfer_categories = st.multiselect("Transfer categories (exclude from totals)", options=cats, default=transfer_default, key="transfer_cats")
...
with c1:
    st.subheader("Review Queue (Unmapped / Ambiguous / Auto-picked)")
    rq = mapped[(mapped["month"] == month) & (~mapped["mapping_status"].isin(["MAPPED"]))].copy()
